...
class TestTBGroq(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        if not os.environ.get("GROQ_API_KEY"):
            raise unittest.SkipTest("Skipping all tests because GROQ_API_KEY environment variable is not set.")

    def test_010_standard_query(self):
        """Test Standard Query"""
        # SYNOPSIS TEST
        chat = TB.chat('llama-3.2-1b-preview')
